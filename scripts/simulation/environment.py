...
class ThreeAgentSingleIntegratorEnv:
    ...
    The active manta workflow uses ``ThreeMantaRayEnv`` below.
    """
...
class MantaEnvConfig:
    """Configuration for the active 3-agent manta environment."""
...
class ThreeMantaRayEnv:
    """Stateful 3-agent manta/CPG environment used by current runs.

    State uses shape ``(3, 7)`` with rows
    ``[x, y, theta, p_L, q_L, p_R, q_R]``. Controls use shape ``(3, 2)`` as
    ...
    @property
    def state(self) -> np.ndarray:
        """Return current state array with shape ``(3, 7)``."""
        return self._state.copy()
    ...
    def step(self, control: np.ndarray | None = None) -> np.ndarray:
        ...
        u = (
            np.zeros((3, 2), dtype=float)
            if control is None
            else _as_control_array(control)
        )
...
def manta_rollout(
    ...
    state_hist = np.zeros((horizon + 1, 3, 7), dtype=float)
    state_hist[0] = env.reset()
    for k in range(horizon):
        state_hist[k + 1] = env.step(control_hist[k])
    return state_hist, control_hist
...
def _as_manta_array(arr: np.ndarray) -> np.ndarray:
    """Validate and cast array to shape ``(3, 7)`` float."""
    out = np.asarray(arr, dtype=float)
    if out.shape != (3, 7):
        raise ValueError(f"expected shape (3, 7), got {out.shape}")
    return out


def _as_control_array(arr: np.ndarray) -> np.ndarray:
    """Validate and cast array to shape ``(3, 2)`` float."""
    out = np.asarray(arr, dtype=float)
    if out.shape != (3, 2):
        raise ValueError(f"expected shape (3, 2), got {out.shape}")
